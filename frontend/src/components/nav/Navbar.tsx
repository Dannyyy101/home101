...
export default function Navbar() {
    ...
    return (
        <nav className="sticky top-0 left-0 w-screen h-16 bg-background-300 flex z-50">
            <section className="ml-4 w-1/2 flex justify-start items-center md:w-1/3"><Link
                className="font-bold text-xl hover:underline"
                href={"/"}>Home101</Link></section>
            <section className="w-1/3 hidden justify-center items-center md:flex">{routes.map((route) =>
                <Link className="font-bold text-xl hover:underline px-8 w-18 text-center"
                      key={route.link}
